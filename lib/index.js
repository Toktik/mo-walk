...
internals.tryToResolveESM = async (filepath) => {
    ...
    try {
        ...
        // When mocking with testdouble.js (using their node loader) import returns ENOENT instead of ERR_MODE_NOT_FOUND
        Assert.ok((err.code === 'ERR_MODULE_NOT_FOUND' || err.code === 'ENOENT') && err.message.includes(`'${filepath}'`), err);
    }
};

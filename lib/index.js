...
exports.walk = async (mo, path, options) => {
    ...
    Assert(typeof visit === 'function', 'Please specify options.visit as a function');
    ...
    const walkDirectory = async (dirname) => {
        ...
        if (stopAtIndexes && indexes.length) {
            Assert(indexes.length === 1, `Multiple index entries found in ${dirname}: ${indexes.map((e) => e.name).join(', ')}`);
